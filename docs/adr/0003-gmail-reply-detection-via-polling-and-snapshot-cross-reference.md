...
> **Patch note 2026-05-05:** The detector now distinguishes asynchronous bounces (DSNs) from customer replies by the local-part of the inbound From address — `mailer-daemon` and `postmaster` are recognized as bounce senders. A bounce flips the step to `email_delivery_status: :bounced`, stops the parent instance with `:stopped_on_delivery_issue`, and sets `JobProposal.status_overlay = "delivery_issue"`. Replies still flip to `:stopped_on_reply` and `"customer_waiting"` as before. The same `gmail_reply_payload` column captures the inbound message JSON in both cases (kept generically named — it is "the inbound message that triggered the stop"). Customer-typed addresses that pass Gmail's send-time validation but bounce later (e.g. `alice@xeample.com` typo) are now caught by this path.
