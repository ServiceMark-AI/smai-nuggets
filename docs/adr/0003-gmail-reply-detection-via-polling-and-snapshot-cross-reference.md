...
### 4. Per-instance latest-step polling

The query returns the *most recent sent step* per campaign instance (Postgres `DISTINCT ON (campaign_instance_id) ... ORDER BY created_at DESC`). A campaign with N sent steps could in principle have N distinct threads to watch, but only the most recent one matters for reply detection — once the customer replies to anything, the campaign stops, and the operator picks up the conversation by hand from there.
...
5. **Poll every step's thread, not just the latest per instance.** Rejected. A campaign that sent 5 steps would multiply Gmail API calls 5x with no reply-detection benefit — once the customer replies to *any* thread, the campaign stops and the operator takes over. The latest-step `DISTINCT ON` query keeps the poll set proportional to active campaigns, not total sent emails.
